    ...
    <div className="flex flex-col lg:flex-row gap-8 pb-10 min-h-screen bg-[#F0F7FF]/50 p-6 rounded-[40px]">
      ...
      <div className="flex-1 space-y-8">
        <div className="space-y-2">
          <h1 className="text-4xl font-black tracking-tight text-slate-900">Danh sách bài kiểm tra</h1>
          <p className="font-medium text-slate-500">Hoàn thành các bài đánh giá để theo dõi tiến độ học tập.</p>
        </div>

        {/* Filters & Tabs */}
        <div className="space-y-6">
          <div className="flex flex-wrap gap-2 p-1 border border-white shadow-sm bg-white/60 backdrop-blur-md rounded-2xl w-fit">
          ...
          <div className="relative max-w-md group">
            <Search className="absolute transition-colors -translate-y-1/2 left-4 top-1/2 text-slate-300 group-focus-within:text-blue-500" size={18} />
            <input
              type="text"
              placeholder="Tìm kiếm bài thi..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full py-4 pl-12 pr-6 font-bold transition-all bg-white border shadow-sm outline-none border-slate-100 rounded-2xl focus:border-blue-300 focus:ring-4 focus:ring-blue-50 text-slate-700 placeholder:text-slate-300"
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="border border-white h-28 bg-white/40 rounded-3xl animate-pulse" />
            ))
            ...
            filteredTests.map((test) => (
              <div
                ...
                className={`group flex items-center gap-6 p-6 rounded-3xl border transition-all cursor-pointer relative overflow-hidden ${selectedTestId === test.id
              ...
              >
                ...
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 ${test.status === 'HOÀN THÀNH' ? 'bg-emerald-50 text-emerald-500' :
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <h3 className="text-lg font-black leading-tight tracking-tight transition-colors text-slate-800 group-hover:text-blue-600">
                    {test.title}
                  </h3>
                ...
                <div className="mr-2 transition-colors text-slate-300 group-hover:text-blue-500">
            ...
            <div className="py-20 text-center bg-white/40 rounded-[40px] border border-dashed border-slate-200">
              ...
              <p className="font-bold text-slate-400">Không tìm thấy bài kiểm tra nào.</p>
            </div>
          )}
        </div>
      </div>
          ...
          {selectedTest ? (
            <>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-4 p-5 border bg-blue-50/50 rounded-3xl border-blue-100/50">
              ...
              {/* Notice */}
              <div className="flex gap-4 p-4 border bg-amber-50 rounded-2xl border-amber-100">
            ...
            <div className="py-40 space-y-4 text-center">
              <div className="flex items-center justify-center w-20 h-20 mx-auto rounded-full bg-slate-50 text-slate-200">
                <BookOpen size={40} />
              </div>
              <p className="font-bold text-slate-400">Chọn một bài thi để xem chi tiết</p>
            </div>
          )}
        </div>
      </div>
