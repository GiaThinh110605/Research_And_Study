...
interface HighlightItem {
  ...
  const highlightColors = [
    { value: 'yellow', label: 'Vang', className: 'bg-yellow-200 text-yellow-900' },
    { value: 'green', label: 'Xanh la', className: 'bg-emerald-200 text-emerald-900' },
    { value: 'blue', label: 'Xanh duong', className: 'bg-sky-200 text-sky-900' },
    { value: 'pink', label: 'Hong', className: 'bg-pink-200 text-pink-900' },
    { value: 'purple', label: 'Tim', className: 'bg-violet-200 text-violet-900' },
  ];
                                      ...
                                      <div key={reply.id} className="flex gap-2.5">
                                        ...
                                        <div className="flex-1">
                                          ...
                                          <div className="flex items-center gap-3 mt-1 ml-2 text-[10px] font-bold text-slate-500">
                                            ...
                                            <div className="relative group/reaction">
                                              <button
                                                ...
                                                className={`hover:text-[#3B66F5] transition-colors py-0.5 ${reply.my_reaction ? 'text-[#3B66F5]' : ''}`}
                                              >
                                            ...
                                            <button onClick={() => handleReplyToggle(reply.id)} className="hover:text-[#3B66F5]">Phản hồi</button>
                                          </div>
                                          ...
                                          {reply.replies && reply.replies.length > 0 && (
                                            <div className="mt-3 space-y-2 pl-4 border-l border-slate-100">
                                              {reply.replies.map((child) => (
                                                <div key={child.id} className="flex gap-2">
                                                  <div className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-[9px] shrink-0">
                                                    {child.user ? getInitials(child.user.full_name) : 'A'}
                                                  </div>
                                                  <div className="flex-1">
                                                    <div className="bg-white rounded-xl border border-slate-100 px-3 py-2 inline-block">
                                                      <span className="font-bold text-[11px] text-slate-800 mr-2">{child.user?.full_name || 'Ẩn danh'}</span>
                                                      <span className="text-[12px] text-slate-700">{child.content}</span>
                                                    </div>
                                                    <div className="flex items-center gap-3 mt-1 ml-2 text-[10px] font-bold text-slate-500">
                                                      <span className="font-normal opacity-70">{timeAgo(child.created_at)}</span>
                                                      <div className="relative group/reaction">
                                                        <button
                                                          onClick={() => handleReaction(child.id, '👍')}
                                                          className={`hover:text-[#3B66F5] transition-colors py-0.5 ${child.my_reaction ? 'text-[#3B66F5]' : ''}`}
                                                        >
                                                      ...
                                                      <button onClick={() => handleReplyToggle(child.id)} className="hover:text-[#3B66F5]">Phản hồi</button>
                                                    </div>
